...
notLegalToDraw = function(locx, locy) {
  var index, line, lines, name, path, xMouse, yMouse;
...
drawBranch = function() {
  var color, index, line, lines, name, path, s, s2, _results;
  _results = [];
  for (name in branchs) {
    path = branchs[name];
    color = path.color;
    ...
    _results.push((function() {
      ...
      for (index in lines) {
        line = lines[index];
        _results2.push(line.y !== line.y2 ? (g.setStrokeStyle(50).beginStroke(color).moveTo(line.x * 50 + 25, line.y * 50).lineTo(line.x2 * 50 + 25, line.y2 * 50), s = new Shape(g), stage.addChild(s)) : (g.setStrokeStyle(50).beginStroke(color).moveTo(line.x * 50, line.y * 50 + 25).lineTo(line.x2 * 50, line.y2 * 50 + 25), s2 = new Shape(g), stage.addChild(s2)));
      }
      return _results2;
    })());
  }
